...
db.sequelize.sync().then(() => {
  ...
  io.on('connection', async (socket) => {
    ...
    try {
      ...
      const onlineUsers = await db.Users.findAll({
        where: { isOnline: true },
        attributes: ['id']
      });
      ...
      socket.emit('initialOnlineUsers', {
        users: onlineUsers.map(user => user.id)
      });
 ...
 socket.on('sendMessage', async ({ roomId, UserDetails, content, replyToMessageId, isMedia, caption }) => {
  ...
  try {
    ...
    // After ensuring sender is joined, also join the other user if online
    ...
    socket.on('deleteMessage', async ({ messageId, roomId, deleteForEveryone = false }) => {
      try {
