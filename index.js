...
if (process.env.NODE_ENV !== 'production') {
  app.use((req, res, next) => {
    const delay = Math.floor(Math.random() * 500) + 0; // Adjust delay as needed
    setTimeout(() => {
      next();
    }, delay);
  });
...
async function initializeDatabase() {
  try {
    ...
    // Check if the Queries table exists already
    const [results] = await db.sequelize.query(
      "SELECT table_name FROM information_schema.tables WHERE table_name = 'Queries'"
    );
    
    if (results.length === 0) {
      
      // Synchronize just the Queries model with the database
      await db.Queries.sync({ alter: true });
    } else {
    }
    
    // You could also sync all models if needed:
    // await db.sequelize.sync(); // Syncs all models
