...
async function detectLocations(content) {
  ...
  if (!content || typeof content !== 'string') {
    if (DEBUG_LOCATION) console.log("Cannot detect location: missing or invalid content");
    return null;
  }
  
  if (DEBUG_LOCATION) console.log(`Detecting location in: "${content}"`);
  
  // Use the enhanced city detector
  const location = cityDetector.detectLocation(content);
  
  if (location) {
    if (DEBUG_LOCATION) console.log(`Location detected: ${location.city}, ${location.state}`);
    return location;
  }
  
  if (DEBUG_LOCATION) console.log("No location detected");
  return null;
}
...
router.post('/test-location-detection', validateToken, async (req, res) => {
  ...
  try {
    console.log(`Testing location detection for: "${text}"`);
...
router.get('/shorts-feed', validateToken, async (req, res) => {
  console.log('==== HYBRID SHORTS FEED STARTED ====');
  ...
  try {
    ...
    console.log(`Pagination: limit=${limit}, lastScore=${lastScore}, lastCreatedAt=${lastCreatedAt?.toISOString()}`);
    console.log(`lastSeenShortTimestamp: ${lastSeenShortTimestamp?.toISOString() || 'Not provided'}`);
    console.log(`firstShortId: ${firstShortId || 'Not provided'}`);
    ...
    console.log(`Found ${followingIds.length} followed users`);
    
    // Step 3: Get IDs of shorts the user has already viewed
    console.log('Fetching viewed shorts history...');
    ...
    console.log(`User has viewed ${viewedShortIds.size} shorts`);
    
    // Handle special case: if firstShortId is provided, fetch that short first
    let specificShort = null;
    if (firstShortId) {
      console.log(`Fetching specific requested short: ${firstShortId}`);
      ...
      if (!specificShort) {
        console.log(`Requested short ${firstShortId} not found`);
      } else {
        console.log(`Found requested short: ${firstShortId}`);
      }
    }
    ...
    if (lastSeenShortTimestamp && (mode === 'hybrid' || mode === 'unviewed')) {
      console.log(`Fetching fresh unviewed shorts created after: ${lastSeenShortTimestamp.toISOString()}`);
      ...
      console.log(`Found ${freshUnviewedShorts.length} fresh unviewed shorts`);
    }
    ...
    if (remainingSlotsAfterFresh > 0 && (mode === 'hybrid' || mode === 'unviewed')) {
      console.log(`Fetching ${remainingSlotsAfterFresh} standard unviewed shorts`);
      ...
      if (followingIds.length > 0) {
        const followedUnviewedShorts = await Post.findAll({
          where: { 
            isShort: true,
        ...
        console.log(`Found ${followedUnviewedShorts.length} unviewed shorts from followed users`);
      ...
      if (remainingSlotsForDiscovery > 0) {
        const discoveryUnviewedShorts = await Post.findAll({
          where: { 
            isShort: true,
            authorId: { 
              [Op.and]: [
        ...
        console.log(`Found ${discoveryUnviewedShorts.length} discovery unviewed shorts`);
    ...
    if ((totalUnviewedCount < limit) || mode === 'viewed') {
      ...
      console.log(`Need ${viewedShortsNeeded} viewed shorts to fill the feed`);
      ...
      console.log(`Found ${viewedShortsRows.length} previously viewed shorts`);
    ...
    console.log('Processing shorts to include interaction data...');
    ...
    console.log('Formatting shorts response...');
    ...
    console.log(`Response ready: ${formattedShorts.length} shorts (${scoredFreshShorts.length} fresh, ${scoredStandardShorts.length} standard, ${scoredViewedShorts.length} viewed)`);
    console.log('==== SHORTS FEED ROUTE COMPLETED SUCCESSFULLY ====');
...
router.get('/user-posts', validateToken, async (req, res) => {
  console.log('==== USER POSTS (LIKED/SAVED) ROUTE STARTED ====');
  try {
    const userId = req.user.id;
    ...
    console.log(`Fetching ${type} ${isShort ? 'shorts' : 'posts'} for user ${userId}`);
    console.log(`Pagination: page=${page}, limit=${limit}, offset=${offset}`);
    console.log(`Filters: type=${type}, isShort=${isShort}`);
    ...
    console.log(`Total ${isShort ? 'shorts' : 'posts'} ${type}: ${totalCount}`);
    ...
    console.log(`Retrieved ${interactions.length} ${isShort ? 'shorts' : 'posts'} interactions`);
    ...
    console.log(`Response ready: ${formattedPosts.length} ${isShort ? 'shorts' : 'posts'}`);
    console.log('==== USER POSTS ROUTE COMPLETED SUCCESSFULLY ====');
...
router.get('/feed', validateToken, async (req, res) => {
  console.log('==== FEED ROUTE STARTED ====');
  console.log(`User ID: ${req.user?.id}`);
  ...
  try {
    ...
    const followingIds = following.map(follow => follow.followingId);
    console.log(`Found ${followingIds.length} followed users:`, followingIds);
    
    // Add the user's own ID to include their posts in the feed
    const allRelevantUserIds = [...followingIds, userId];
    console.log(`Total relevant userIds for feed: ${allRelevantUserIds.length}`);
    
    // Fetch posts from followed users and user's own posts
    // Adding isShort: false to exclude shorts
    console.log('Fetching posts (excluding shorts)...');
    ...
    console.log(`Retrieved ${posts.length} regular posts (shorts excluded)`);
    
    // Also update the total count query to exclude shorts
    console.log('Calculating total post count (excluding shorts)...');
    ...
    console.log(`Total regular posts available: ${totalCount}`);
    ...
    if (needBackfill) {
      console.log('Need to backfill with ranked algorithm...');
      ...
      console.log(`Found ${priorityUserIds.size} priority users for boosting rankings`);
      ...
      console.log(`Retrieved ${backfillPool.length} posts for ranking and backfill`);
      ...
      if (backfillPool.length > 0) {
        ...
        console.log(`After ranking, ${priorityPostsInTop} out of top ${backfillLimit} posts are from priority accounts`);
        ...
        console.log(`Backfill scores - Highest: ${scoreStats.highest.toFixed(4)}, Lowest: ${scoreStats.lowest.toFixed(4)}, Avg: ${scoreStats.average.toFixed(4)}`);
      }
    }
    ...
    console.log(`Fetching interaction data for ${postIds.length} posts:`, postIds);
    
    // Process likes, saves, and follows in parallel for efficiency
    console.log('Starting parallel queries for user interactions...');
    const [likeRecords, saveRecords, followRecords] = await Promise.all([
      PostLike.findAll({
        where: { userId, postId: { [Op.in]: postIds } },
        attributes: ['postId']
    ...
    console.log(`Found ${likeRecords.length} liked posts`);
    console.log(`Found ${saveRecords.length} saved posts`);
    console.log(`Found ${followRecords.length} followed authors`);
    ...
    console.log('Formatting posts response...');
    ...
    console.log(`Response ready: ${formattedPosts.length} posts (${posts.length} regular, ${backfillPosts.length} backfill)`);
    console.log('==== FEED ROUTE COMPLETED SUCCESSFULLY ====');
  ...
  try {
    ...
    const stories = userData.Stories.map(story => ({
      ...
      elements: (story.metadata || []).map(meta => ({
        ...
        offset_x: meta.offset_x || 0,
        offset_y: meta.offset_y || 0,
        dimensions: {
          width: meta.width || 0,
...
router.get("/summary/:id", async (req, res) => {
  ...
  try {
    ...
    let displayTitle = post.title || post.content;
...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, './cloud/Postthumbnail'); // Directory where images will be stored
  },
...
router.post('/upload-images', uploadPostImages.single('tempPostImages'), async (req, res) => {
  try {
    ...
    // Return file name so frontend can construct the final URL
...
// Set up multer storage for short media uploads
const storageShortMedia = multer.diskStorage({
  destination: function (req, file, cb) {
    if (file.mimetype.startsWith("video/")) {
      cb(null, "./cloud/shortVideos");
    } else if (file.mimetype.startsWith("image/")) {
      cb(null, "./cloud/shortImages");
    } else {
...
const uploadShortMedia = multer({ 
  ...
  limits: {
    fileSize: 120 * 1024 * 1024, // 50MB limit per file
  }
});

// Replace the existing single-file short creation route with multi-file support
router.post("/create-short", validateToken, uploadShortMedia.array("media", 4), async (req, res) => {
  ...
  try {
    ...
    const categories = req.body.categories ? JSON.parse(req.body.categories) : ["General"];
    ...
    if (!hasContent && !hasMedia) {
      return res.status(400).json({ error: "Short must have either content or media." });
    }
    
    // For shorts, media is required regardless of content
    if (!hasMedia) {
      return res.status(400).json({ error: "At least one media file is required for shorts." });
    }

    // Check if only one video is included if any
    const videoFiles = req.files.filter(file => file.mimetype.startsWith('video/'));
    if (videoFiles.length > 1) {
      return res.status(400).json({ error: "Only one video file allowed per short" });
    }
    ...
    // Use the first file as the thumbnail (could be improved with better thumbnail selection logic)
    const thumbnail = req.files[0].filename;

    // Add metadata to track aspect ratio - always use 9:16 for shorts
    const mediaMetadata = {
      aspectRatio: "9:16",
    ...
    // Normalize hashtags and mentions (remove '#'/'@' and lower-case)
    ...
    // Detect location in content with improved city detection
    let location = null;
    try {
      console.log("Content for location detection:", content);
    ...
    const newShort = await Post.create({
      content: content,
      authorId: userId,
      media: mediaObjects, // Store multiple media items
      thumbnail: thumbnail,
      isShort: true,
      categories: categories.join(","),
      mediaMetadata: mediaMetadata,
      location: location, // Store detected location
    }, { transaction: t });
    ...
    // Process hashtags: update Hashtag table and create PostHashtags relationship.
    for (const tag of normalizedHashtags) {
      ...
      // Create a relationship using PostHashtags table.
    ...
    // Process mentions: create Mention records.
...
router.delete('/delete-image/:filename', async (req, res) => {
  try {
    ...
    const filepath = path.join(__dirname, '../cloud/tempPostImages', filename);
...
// Set up multer storage for post media uploads
const storagePostMedia = multer.diskStorage({
  destination: function (req, file, cb) {
    if (file.fieldname === 'postImages') {
      cb(null, './cloud/postImages');
    } else if (file.fieldname === 'postVideos') {
      cb(null, './cloud/postVideos');
    } else {
      cb(null, './cloud/Postthumbnail'); // Fallback for thumbnail
    }
  },
  filename: function (req, file, cb) {
    const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1E9);
...
const uploadPostMedia = multer({
  ...
  fileFilter: (req, file, cb) => {
    if (file.fieldname === 'postImages' && !file.mimetype.startsWith('image/')) {
      return cb(new Error('Only image files are allowed for postImages field'));
    }
    if (file.fieldname === 'postVideos' && !file.mimetype.startsWith('video/')) {
      return cb(new Error('Only video files are allowed for postVideos field'));
    }
    cb(null, true);
  }
});

// Update the create-post route to handle the new media format
router.post('/create-post', validateToken, uploadPostMedia.fields([
  { name: 'postImages', maxCount: 4 },
  { name: 'postVideos', maxCount: 1 },
]), async (req, res) => {
  const { title, content, hashtags, mentions, categories } = req.body;
  const userId = req.user.id;
  ...
  const parsedCategories = categories ? JSON.parse(categories) : ["General"];
  ...
  // Process media files - construct the JSON media array
  const mediaArray = [];
  
  // Add images - preserving original formats
  if (files.postImages) {
    files.postImages.forEach(file => {
      mediaArray.push({
        type: 'image',
        url: file.filename,
        originalName: file.originalname
  ...
  // Add video (if present) - preserving original format
  if (files.postVideos && files.postVideos.length > 0) {
    mediaArray.push({
      type: 'video',
      url: files.postVideos[0].filename,
      originalName: files.postVideos[0].originalName
    });
  }
  ...
  try {
    console.log("Content for location detection:", content);
    location = await detectLocations(content || title);
    console.log("Location detected:", location);
  } catch (error) {
    console.error("Error detecting location:", error);
  }
  ...
  try {
    const newPost = await Post.create({
      ...
      location, // Store detected location
    }, { transaction: t });
...
router.delete('/:id', validateToken, async (req, res) => {
  ...
  try {
    ...
    console.log(`Deleting media files for post ${postId}...`);
    ...
    if (post.media && Array.isArray(post.media)) {
      post.media.forEach(media => {
        try {
          ...
          if (!media.url) {
            console.log(`Skipping media with missing URL in post ${postId}`);
            return;
          }
          
          // Determine correct directory based on media type and post type
          let filePath;
          if (post.isShort) {
            filePath = media.type === 'video' 
              ? path.join(__dirname, '../cloud/shortVideos', media.url)
              : path.join(__dirname, '../cloud/shortImages', media.url);
          } else {
            filePath = media.type === 'video' 
              ? path.join(__dirname, '../cloud/postVideos', media.url)
              : path.join(__dirname, '../cloud/postImages', media.url);
          }
    ...
    if (post.thumbnail && (!post.media || !post.media.some(m => m.url === post.thumbnail))) {
      try {
        const thumbnailPath = path.join(__dirname, '../cloud/Postthumbnail', post.thumbnail);
        ...
        failedFiles.push(post.thumbnail);
      }
    }
    
    console.log(`Post ${postId} deletion complete. Deleted ${deletedFiles.length} files, ${failedFiles.length} failed.`);
