...
- **Unknown where to start / soft “how does X work?”** → `map`(descriptive code-vocab query, k=10) → **required next** `pack_context`(same query, `suggested_seed`, mode=lean). Do **not** stop after map-only. Heatmap + hot bodies come from pack.
...
- Budget: ≤3 Scubiee locate calls for the first beat (map→pack→expand); then edit. Avoid token dumps and parallel explore thrash.
