...
router.get('/google/callback',
  ...
  async (req, res) => {
    // First check if we found an existing user in the auth strategy
    if (req.existingUser) {
    ...
    // If no existing user found in auth strategy, check session/cookies
    ...
    if (userData && userData.email) {
      try {
        // Double check if user exists (in case strategy check missed it)
        ...
        if (user) {
          ...
          return res.redirect(`${Domain}/complete`);
        }
      } catch (error) {
        return res.redirect(`${Domain}/complete`);
      }
...
router.get('/github/callback',
  ...
  async (req, res) => {
    ...
    if (tempUserData) {
      try {
        const user = await Users.findOne({ where: { email: tempUserData.email } });
        if (user) {
          const token = generateToken(user);
          res.cookie('access_token', token, { httpOnly: true, secure: false, maxAge: 1209600000 }); // 2 weeks
          res.clearCookie('temp_user_data'); // Clear temporary data
          res.redirect('Domain/');
        } else {
          res.redirect('Domain/complete');
        }
      } catch (error) {
        res.redirect('Domain/complete');
      }
    } else {
      res.redirect('Domain/complete'); // Redirect to complete route if temp user data is null
    }
  }
);
...
router.post('/complete-profile', upload.single('profilePic'), async (req, res) => {
  try {
    ...
    const { username, complete, firstName, lastName, categories } = req.body;
    const tempUserData = req.cookies.temp_user_data ? JSON.parse(req.cookies.temp_user_data) : null;
...
router.get('/profile-info', validateToken, async (req, res) => {
  ...
  try {
    // Find user with all relevant information
    const user = await Users.findOne({
