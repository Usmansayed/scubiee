...
router.post('/create-post', validateToken, uploadPostMedia.fields([
  ...
  const { content, hashtags, mentions, categories } = req.body;
  const userId = req.user.id;
  ...
  if (files.postVideos && files.postVideos.length > 0) {
    ...
    try {
      const result = await uploadToB2(file.buffer, filename, 'postVideoss');
      ...
      for (const media of post.media) {
        try {
          ...
          if (post.isShort) {
            ...
            folder = media.type === 'video' ? 'postVideoss' : 'postImages';
          }
