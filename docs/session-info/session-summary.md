...
**Branch:** `feat/production-certification` @ **`e59fb8a`**  
**`main`** was reset to the **same Mac tip** (`e59fb8a`) — no extra merge commit. Do not re-merge this worktree onto `main` as a second history.
...
| **0.2.14** | Windows `ctx setup` pip **PIPE deadlock** fix (bar stuck at 31%). |

PyPI: https://pypi.org/project/scubiee/0.2.14/

## What is solved (product)

- Agent locate surface is **phase** (`map` / `focus` / `grep` / `glob` / `workspace` / `status`).
- Mac GPU for **production indexing** is **MLX FP16 on Metal**, measured ~2–3× CPU on M5 benches (see `docs/mlx-m5-index-report.md`, `docs/macbook-embed-speed.md`).
- After `ctx sync`, running daemon **reloads** search (generation bump) — not stale in-memory FAISS.
...
6. Phase grep/glob so agents do not mix native Grep with MCP.
...
7. **Windows:** some MCP/SDK tests were failing earlier (not Mac-blocking). Re-run after 0.2.13.
8. **`feat/live-reindexing`** not merged into this line unless someone does it separately.
9. **GitHub `PYPI_API_TOKEN`** — CI publish optional; last uploads were manual twine from `.env`.
10. **CoreML path** still in tree; Mac default is MLX. Do not revive CoreML as default without a real-device pass that shows **no** CPU fallback in logs.
11. README still mentions git install `@v0.2.6` in one fallback snippet — should say `0.2.13` when docs are next touched.
12. Bench junk: `docs/bench-*.err.log`, `:memory:.ses` — do not commit secrets or leftover logs.

## New-session prompt (paste)

```
Read docs/session-info/README.md, session-summary.md, issue-log.md, then docs/engg/.
Branch feat/production-certification @ e59fb8a, PyPI scubiee 0.2.13.
Mac venv is ~/scubiee. GPU path is MLX FP16. Do not Path.resolve MCP python on Darwin.
