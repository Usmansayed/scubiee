"""Registry of AI coding tools: global MCP + rules paths only.

See docs/connect-global-mcp-research.md for sources (Win/Mac) researched 2026-08-23.

connect writes user-global configs only — no project files, no CTX_REPO pin.
"""
...
TOOLS: list[ToolDef] = [
    ...
    ToolDef(
        ...
        notes="Global ~/.kiro only; no workspace .kiro writes.",
    ),
    ToolDef(
        name="Windsurf",
        slug="windsurf",
    ...
    ToolDef(
        ...
        notes=(
            ...
            "(mcpServers+type:local) + global instructions under ~/.copilot/."
        ),
    ),
    ToolDef(
        name="Cline",
        ...
        notes="Writes VS Code globalStorage + ~/.cline CLI MCP settings.",
    ),
    ToolDef(
        name="Roo Code",
        slug="roo-code",
        ...
        rule_format="none",
        notes="VS Code globalStorage only; project .roo rules skipped for global connect.",
    ),
    ToolDef(
        name="Continue",
        slug="continue",
...
def resolve_mcp_project_path(tool: ToolDef, repo: Path | None) -> Path | None:
    """Global-only install: never writes project MCP."""
    return None
