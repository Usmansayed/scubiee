...
def test_bootstrap_budget_defaults() -> None:
    ...
    assert b.rss_cap_mb == BOOTSTRAP_RSS_CAP_MB
...
def test_background_budget_defaults() -> None:
    ...
    assert b.rss_cap_mb == BACKGROUND_RSS_CAP_MB
    assert b.mlx_batch == 16
    assert b.mlx_cache_mb == 64
...
def test_resolve_background_vs_bootstrap(tmp_path: Path) -> None:
    ...
    assert resolve_index_memory_budget(background=True).mode == "background"
    assert resolve_index_memory_budget(background=False, store=store).mode == "bootstrap"
...
def test_apply_index_memory_budget_respects_existing_env(monkeypatch) -> None:
    ...
    try:
        ...
        assert os.environ["CTX_CE_EMB_BATCH_CEILING"] == "16"
    finally:
